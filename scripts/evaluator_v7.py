...
from telegram_notifier import send_telegram_message  # assicurati che funzioni
...
SUMMARY_LOG = os.path.join(EVALUATIONS_DIR, 'evaluation_summary.csv')
...
def evaluate_predictions():
    ...
    for file_path in sorted(prediction_files):
        ...
        # === Riepilogo
        long_df = df_result[df_result['prediction'] == 'LONG']
        short_df = df_result[df_result['prediction'] == 'SHORT']

        long_total = len(long_df)
        long_correct = long_df['score'].sum()

        short_total = len(short_df)
        short_correct = short_df['score'].sum()

        summary_row = {
            "timestamp": file_time.strftime("%Y-%m-%d %H:%M:%S"),
            "long_total": long_total,
            "long_correct": int(long_correct),
            "short_total": short_total,
            "short_correct": int(short_correct)
        }

        pd.DataFrame([summary_row]).to_csv(SUMMARY_LOG, mode='a', index=False, header=not os.path.exists(SUMMARY_LOG))
        print(f"[INFO] Riepilogo aggiornato in {SUMMARY_LOG}")

        # === Telegram message
        msg = (
            f"Ciclo valutazione {file_time.strftime('%H:%M')} UTC\n"
            f"LONG: {int(long_correct)}/{long_total} corretti\n"
            f"SHORT: {int(short_correct)}/{short_total} corretti"
        )
        send_telegram_message(msg)

        # === Log simboli mancanti
        if missing_symbols:
